...
- Column 2: Physical Input - The actual button/axis name from controller manual (e.g., "Stick Y-Axis", "Trigger", "TDC Slew Up", "Left Throttle Lever")
- Column 3: Device ID - ONLY the device identifier (e.g., "JOY_Y", "JOY_BTN1", "JOY_SLIDER1") - NO action descriptions
- Column 4: {simulator} Command - Exact command for the simulator (e.g., "iCommandPlanePitch")
- Column 5: Function & Rationale - Detailed explanation including aircraft function, ergonomic reasoning, technical notes (curves, dead zones), combat effectiveness

**Example Rows:**
| **Flight Stick** | Stick Y-Axis | JOY_Y | iCommandPlanePitch | Pitch control for elevator. Standard flight control mapping. Recommended curve: 15 for smoother input. |
| **Throttle** | Left Throttle Lever | JOY_SLIDER1 | iCommandPlaneThrustLeft | Left engine power control. Critical for carrier operations and differential thrust maneuvers. |
| **Throttle** | TDC Slew Up | JOY_BTN25 | iCommandTDCUp | Target Designator Cursor up movement. Primary sensor control for radar/FLIR targeting. Most frequently used combat control. |

**CRITICAL MAPPING PRIORITIES:**
1. **Flight Stick Device:** 
   - Primary flight axes (pitch/roll/yaw) 
   - Weapon trigger (two-stage if available)
   - Nose wheel steering button
   - Trim hat switches
   - Weapon release button

2. **Throttle Device:** 
   - Throttle axes (left/right engines or single)
   - TDC (Throttle Designator Control) slew control
   - Sensor control switches
   - Countermeasure dispense
   - Speed brake switch
   - Master arm switch
   - Radio PTT

3. **Additional Devices:** 
   - Rudder pedals (yaw, brakes)
   - Control panels (UFC, displays)
...
{f"Aircraft Manual Content:\n{aircraft_text[:12000]}" if aircraft_text else "No aircraft manual provided. Use general knowledge of this aircraft."}

---

## FINAL CRITICAL INSTRUCTIONS:
1. **Device ID Column** - MUST contain ONLY the device identifier (JOY_Y, JOY_BTN1, etc.) - NO action descriptions
...
6. **Include INSTALLATION guide** - Clear numbered steps
7. **Use EXACT command syntax** for {simulator}
8. **Add comments/documentation** in config file explaining each section
...
# --- Main Application UI ---
st.set_page_config(page_title="GAMECHANGER", page_icon="✈️", layout="wide")
